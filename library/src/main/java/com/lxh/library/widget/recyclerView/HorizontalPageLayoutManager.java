...
public class HorizontalPageLayoutManager extends RecyclerView.LayoutManager implements PageDecorationLastJudge {
    ...
    private int getUsableWidth() {
        return getWidth() - getPaddingLeft() - getPaddingRight() - sideWidth;
    }
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        //计算宽高已经使用的量，主要用于后期测量
        itemWidthUsed = ((columns - 1) * itemWidth) + sideWidth;
...
//        pageSize = state.getItemCount() / onePageSize + (state.getItemCount() % onePageSize == 0 ? 0 : 1);
        ...
        //计算可以横向滚动的最大值
        totalWidth = (pageSize - 1) * (getWidth() - sideWidth)-sideWidth;
    ...
    public boolean isLastRow(int index) {
        if (index >= 0 && index < getItemCount()) {
            ...
            indexOfPage++;
            return indexOfPage > (rows - 1) * columns && indexOfPage <= onePageSize;
        }
